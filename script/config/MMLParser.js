class MathML {
	constructor() {
		this.id = "mml";
		this.version = "2.7.1";
		this.directory = MathJax.ElementJax.directory + "/mml";
		this.extensionDir = MathJax.ElementJax.extensionDir + "/mml";
		this.optableDir = MathJax.ElementJax.directory + "/mml/optable";
	}
}
	...
	mml.Augment({
		Init: function () {
			if (arguments.length === 1 && arguments[0].type === "math") {
				...
				this.root = mml.math.apply(this, arguments);
			}
	...
	mml.Parse = MathJax.Object.Subclass({
		...
		Parse: function (h, e) {
			...
			if (typeof h !== "string") {
				...
				j = mml.ParseXML(this.preProcessMath.call(this, h));
				if (j == null) { mml.Error(["ErrorParsingMathML", "Error parsing MathML"]) }
			}
			...
			if (g) {
				mml.Error([
					...
					g.textContent.replace(/This page.*?errors:|XML Parsing Error: |Below is a rendering of the page.*/g, "")
				]);
			}
			if (j.childNodes.length !== 1) {
				mml.Error(["MathMLSingleElement", "MathML must be formed by a single element"]);
			}
			if (j.firstChild.nodeName.toLowerCase() === "html") {
				...
				if (f && f.textContent === "XML parsing error" && f.nextSibling) {
					mml.Error(["ParsingError", "Error parsing MathML: %1", String(f.nextSibling.nodeValue).replace(/fatal parsing error: /, "")]);
				}
			}
			if (j.firstChild.nodeName.toLowerCase().replace(/^[a-z]+:/, "") !== "math") {
				mml.Error(["MathMLRootElement", "MathML must be formed by a <math> element, not %1", "<" + j.firstChild.nodeName + ">"]);
			}
			var i = { math: j.firstChild, script: e };
			mml.DOMfilterHooks.Execute(i);
		...
		MakeMML: function (h) {
			...
			if (mml.config.useMathMLspacing) { f.useMMLspacing = 8 }
			return f;
		},
		TeXAtom: function (g, f) {
			var e = a.TeXAtom().With({ texClass: a.TEXCLASS[g] });
			if (f) { e.movesupsub = e.movablelimits = true }
			return e;
		},
		...
		AddAttributes: function (g, j) {
			...
			for (var h = 0, e = j.attributes.length; h < e; h++) {
				...
				if (f == "xlink:href") { f = "href" }
				if (f.match(/:/)) { continue }
				if (f.match(/^_moz-math-((column|row)(align|line)|font-style)$/)) { continue }
		...
		AddChildren: function (e, g) {
			for (var k = 0, j = g.childNodes.length; k < j; k++) {
				...
				if (f.nodeName === "#comment") { continue }
				if (f.nodeName === "#text") {
					if ((e.isToken || e.isChars) && !e.mmlSelfClosing) {
						...
						if (f.nodeValue.match(/\S/)) {
							mml.Error(["UnexpectedTextNode", "Unexpected text node: %1", "'" + f.nodeValue + "'"]);
						}
					}
				} else {
		...
		replaceEntity: function (g, f) {
			if (f.match(/^(lt|amp|quot)$/)) { return g }
			if (mml.Parse.Entity[f]) { return mml.Parse.Entity[f] }
			var h = f.charAt(0).toLowerCase();
			var e = f.match(/^[a-zA-Z](fr|scr|opf)$/);
			if (e) { h = e[1] }
			if (!mml.Parse.loaded[h]) {
				mml.Parse.loaded[h] = true;
				MathJax.Hub.RestartAfter(MathJax.Ajax.Require(mml.entityDir + "/" + h + ".js"));
			}
			return g;
		}
	}, {
		...
		createParser: function () {
			if (window.DOMParser) {
				...
				if (window.ActiveXObject) {
					...
					if (!this.parser) {
						MathJax.Localization.Try(this.parserCreationError); return (this.parseError);
					}
					this.parser.async = false; return (this.parseMS);
				}
			}
