function PREVIEW_HTML(ajax, hub, html, pre) {
	var h; var j, a, d;
	...
	pre.Augment({
		...
		Startup: function () {
			...
			var n = html.addElement(document.body, "div", { style: { width: "5in" } });
			this.pxPerInch = n.offsetWidth / 5; n.parentNode.removeChild(n);
		...
		preTranslate: function (p) {
			...
			for (t = 0; t < q; t++) {
				...
				if (n.PHTML.display) {
					o = html.Element("div", { className: "MathJax_PHTML_Display" });
					o.appendChild(v);
				}
		...
		getJaxFromMath: function (n) {
			...
			return hub.getJaxFor(n);
		},
		getHoverSpan: function (n, _o) {
			return n.root.PHTMLspanElement();
		},
		getHoverBBox: function (n, _q, _r) {
			var s = n.root.PHTML, p = n.PHTML.outerEm;
			var o = { w: s.w * p, h: s.h * p, d: s.d * p };
			if (s.width) { o.width = s.width }
			return o;
		},
		Zoom: function (o, u, s, _n, _r) {
			u.className = "MathJax";
			this.idPostfix = "-zoom";
			o.root.toPHTML(u, u);
			this.idPostfix = "";
			u.style.position = "absolute";
			if (!width) { s.style.position = "absolute" }
			var t = u.offsetWidth,
				q = u.offsetHeight,
				v = s.offsetHeight,
				p = s.offsetWidth;
			if (p === 0) { p = s.parentNode.offsetWidth }
		...
		Remove: function (n) {
			...
			if (o) {
				if (n.PHTML.display) { o = o.parentNode }
		...
		length2em: function (r, p) {
			...
			if (r === "") { return "" }
			if (r === h.SIZE.NORMAL) { return 1 }
			if (r === h.SIZE.BIG) { return 2 }
			if (r === h.SIZE.SMALL) { return 0.71 }
			if (this.MATHSPACE[r]) { return this.MATHSPACE[r] }
			var o = r.match(/^\s*([-+]?(?:\.\d+|\d+(?:\.\d*)?))?(pt|em|ex|mu|px|pc|in|mm|cm|%)?/);
			var n = parseFloat(o[1] || "1"), q = o[2];
			if (p == null) { p = 1 }
			if (q === "em") { return n }
			if (q === "ex") { return n * this.TeX.x_height }
			if (q === "%") { return n / 100 * p }
			if (q === "px") { return n / this.em }
			if (q === "pt") { return n / 10 }
			if (q === "pc") { return n * 1.2 }
			if (q === "in") { return n * this.pxPerInch / this.em }
			if (q === "cm") { return n * this.pxPerInch / this.em / 2.54 }
			if (q === "mm") { return n * this.pxPerInch / this.em / 25.4 }
			if (q === "mu") { return n / 18 }
			return n * p;
		},
		Em: function (n) {
			if (Math.abs(n) < 0.001) { return "0em" }
	...
	MathJax.Hub.Register.StartupHook("mml Jax Ready", function () {
		...
		h.mbase.Augment({
			...
			PHTMLdefaultSpan: function (q, o) {
				...
				if (this.isToken) { this.PHTMLhandleToken(q) }
			...
			PHTMLaddChild: function (p, o, n) {
				...
				if (q) {
					if (n.childSpans) {
						p = html.addElement(p, "span", { className: n.className });
					}
					q.toPreviewHTML(p);
					if (!n.noBBox) {
						this.PHTML.w += q.PHTML.w + q.PHTML.l + q.PHTML.r;
						if (q.PHTML.h > this.PHTML.h) { this.PHTML.h = q.PHTML.h }
						if (q.PHTML.d > this.PHTML.d) { this.PHTML.d = q.PHTML.d }
						if (q.PHTML.t > this.PHTML.t) { this.PHTML.t = q.PHTML.t }
						if (q.PHTML.b > this.PHTML.b) { this.PHTML.b = q.PHTML.b }
					}
				} else {
					if (n.forceChild) { html.addElement(p, "span") }
				}
			},
			...
			PHTMLcreateSpan: function (n) {
				...
				if (!this.PHTMLspanID) { this.PHTMLspanID = pre.GetID() }
				var o = (this.id || "MJXp-Span-" + this.PHTMLspanID);
				return html.addElement(n, "span", { className: "MJXp-" + this.type, id: o });
			},
			...
			PHTMLhandleToken: function (o) {
				...
				if (n.mathvariant !== h.VARIANT.NORMAL) {
					o.className += " " + pre.VARIANT[n.mathvariant];
				}
			},
			PHTMLhandleStyle: function (n) {
				if (this.style) { n.style.cssText = this.style }
			},
			PHTMLhandleColor: function (n) {
				if (this.mathcolor) { n.style.color = this.mathcolor }
				if (this.mathbackground) { n.style.backgroundColor = this.mathbackground }
			},
			PHTMLhandleScriptlevel: function (n) {
				var o = this.Get("scriptlevel");
				if (o) { n.className += " MJXp-script" }
			},
			PHTMLhandleText: function (y, A) {
				...
				for (var s = 0, r = A.length; s < r; s++) {
					...
					if (p < 127) {
						if (v.match(/[A-Za-ehik-or-xz0-9]/)) { u = 0 }
						if (v.match(/[A-HK-Z]/)) {
							x = 0.67;
						} else {
							if (v.match(/[IJ]/)) { x = 0.36 }
						}
						if (v.match(/[acegm-su-z]/)) {
							t = 0.45;
						} else {
							if (v.match(/[ij]/)) { t = 0.75 }
						}
						if (v.match(/[ijlt]/)) { x = 0.28 }
					}
					if (pre.DELIMITERS[v]) {
						x = pre.DELIMITERS[v].w || 0.4;
					}
					if (t > z) { z = t }
					if (u > o) { o = u }
					q += x;
			...
			CoreParent: function () {
				var n = this;
				while (n && n.isEmbellished() && n.CoreMO() === this && !n.isa(h.math)) { n = n.Parent() }
				return n;
			},
			CoreText: function (n) {
				if (!n) { return "" }
		...
		h.mo.Augment({
			toPreviewHTML: function (o) {
				...
				if (n.scriptlevel === 0) {
					this.PHTML.l = pre.length2em(n.lspace);
					this.PHTML.r = pre.length2em(n.rspace);
					o.style.marginLeft = pre.Em(this.PHTML.l);
					o.style.marginRight = pre.Em(this.PHTML.r);
				} else {
					this.PHTML.l = 0.15;
					this.PHTML.r = 0.1;
				}
				if (n.displaystyle && n.largeop) {
					var p = html.Element("span", { className: "MJXp-largeop" });
				...
				if (o && o.isa(h.munderover) && this.CoreText(o.data[o.base]).length === 1) {
					...
					if (q && this === q.CoreMO() && o.Get("accent")) {
						...
						if (n && this === n.CoreMO() && o.Get("accentunder")) {
							r = pre.REMAPACCENTUNDER[s];
						}
					}
					if (r) { s = p.innerHTML = r }
					if (s.match(/[\u02C6-\u02DC\u00A8]/)) {
						this.PHTML.acc = -0.52;
					} else {
						if (s === "\u2192") {
			...
			PHTMLstretchV: function (p, u) {
				...
				n.maxsize = pre.length2em(n.maxsize, t.h + t.d);
				n.minsize = pre.length2em(n.minsize, t.h + t.d);
				l = Math.max(n.minsize, Math.min(n.maxsize, l));
				var s = l / (t.h + t.d - 0.3);
				var q = html.Element("span", { style: { "font-size": pre.Em(s) } });
				if (s > 1.25) {
					var r = Math.ceil(1.25 / s * 10);
					q.className = "MJXp-right MJXp-scale" + r;
					q.style.marginLeft = pre.Em(t.w * (r / 10 - 1) + 0.07);
					t.w *= s * r / 10;
				}
				q.appendChild(o.firstChild);
				o.appendChild(q);
				if (n.symmetric) {
					o.style.verticalAlign = pre.Em(0.25 * (1 - s));
				}
			}
		});
		h.mspace.Augment({
			toPreviewHTML: function (q) {
				...
				var n = pre.length2em(o.width),
					p = pre.length2em(o.height),
					s = pre.length2em(o.depth);
				var r = this.PHTML;
				r.w = n;
				r.h = p;
				r.d = s;
				if (n < 0) {
					if (!pre.lastIsInt) { q.style.marginLeft = pre.Em(n) }
					n = 0;
				}
				q.style.width = pre.Em(n);
				q.style.height = pre.Em(p + s);
				if (s) { q.style.verticalAlign = pre.Em(-s) }
				return q;
			}
		});
		h.mpadded.Augment({
			toPreviewHTML: function (u) {
				...
				if (v.width !== "") {
					...
					if (s.pm) {
						...
						u.style.width = pre.Em(s.len);
					}
				}
				if (v.height !== "") {
					s = this.PHTMLdimen(v.height, "h", 0);
					if (!s.pm) { q += -this.PHTMLbboxFor(0).h }
					q += s.len;
				}
				if (v.depth !== "") {
				...
				if (q) { o.style.marginTop = pre.Em(q) }
				if (n) { o.style.marginBottom = pre.Em(n) }
				if (t) { o.style.marginLeft = pre.Em(t) }
				if (r) { o.style.marginRight = pre.Em(r) }
				if (p) { u.style.verticalAlign = pre.Em(p) }
				return u;
			},
			PHTMLdimen: function (q, r, n) {
				...
				return { len: pre.length2em(q, p) || 0, pm: !!q.match(/^[-+]/) };
			}
		});
		h.munderover.Augment({
			toPreviewHTML: function (r) {
				...
				if (this.data[this.over]) {
					...
					var q = html.Element("span", {}, [["span", { className: "MJXp-over" }]]);
					...
					if (o != null) {
						...
						o = o - p.d + 0.1;
						if (u.t != null) { o += u.t - u.h }
						q.firstChild.firstChild.style.marginBottom = pre.Em(o);
					}
					if (r.firstChild) {
		...
		h.msubsup.Augment({
			toPreviewHTML: function (q) {
				...
				if (!this.data[this.base]) {
					if (q.firstChild) {
						q.insertBefore(html.Element("span"), q.firstChild);
					} else {
						q.appendChild(html.Element("span"));
					}
				}
				var s = this.data[this.base],
					p = this.data[this.sub],
					n = this.data[this.sup];
				if (!s) { s = { bbox: { h: 0.8, d: 0.2 } } }
				q.firstChild.style.marginRight = ".05em";
				var o = Math.max(0.4, s.PHTML.h - 0.4),
					u = Math.max(0.2, s.PHTML.d + 0.1);
				var t = this.PHTML;
				if (n && p) {
					var r = html.Element("span", {
						className: "MJXp-script-box",
						style: {
							height: pre.Em(o + n.PHTML.h * 0.8 + u + p.PHTML.d * 0.8),
							"vertical-align": pre.Em(-u - p.PHTML.d * 0.8)
						}
					}, [
						["span", {}, [["span", {}, [["span", { style: { "margin-bottom": pre.Em(-(n.PHTML.d - 0.05)) } }]]]]],
						["span", {}, [["span", {}, [["span", { style: { "margin-top": pre.Em(-(n.PHTML.h - 0.05)) } }]]]]]
					]);
					...
					if (n) {
						q.lastChild.style.verticalAlign = pre.Em(o);
						...
						if (p) {
							q.lastChild.style.verticalAlign = pre.Em(-u);
		...
		h.mfrac.Augment({
			toPreviewHTML: function (r) {
				...
				var n = html.Element(
					"span", { className: "MJXp-box" },
					[["span", { className: "MJXp-denom" }, [["span", {}, [["span", { className: "MJXp-rule", style: { height: "1em" } }]]], ["span"]]]]
				);
				...
				v.d = p.h + p.d - 0.25;
				v.l = v.r = 0.125;
				o.linethickness = Math.max(0, pre.length2em(o.linethickness || "0", 0));
				if (o.linethickness) {
					var u = n.firstChild.firstChild.firstChild;
					var q = pre.Em(o.linethickness);
					u.style.borderTop = "none";
					u.style.borderBottom = (o.linethickness < 0.15 ? "1px" : q) + " solid";
					u.style.margin = q + " 0";
					q = o.linethickness;
					n.style.marginTop = pre.Em(3 * q - 1.2);
					r.style.verticalAlign = pre.Em(1.5 * q + 0.1);
					v.h += 1.5 * q - 0.1;
					v.d += 1.5 * q;
				} else {
		...
		h.msqrt.Augment({
			toPreviewHTML: function (n) {
				...
				this.PHTMLlayoutRoot(n, n.firstChild); return n
			},
			PHTMLlayoutRoot: function (u, n) {
				var v = this.PHTMLbboxFor(0);
				var q = Math.ceil((v.h + v.d + 0.14) * 100), w = pre.Em(14 / q);
				var r = html.Element("span", { className: "MJXp-surd" }, [["span", { style: { "font-size": q + "%", "margin-top": w } }, ["\u221A"]]]);
				var s = html.Element("span", { className: "MJXp-root" }, [["span", { className: "MJXp-rule", style: { "border-top": ".08em solid" } }]]);
				var p = (1.2 / 2.2) * q / 100;
				if (q > 150) {
					var o = Math.ceil(150 / q * 10);
					r.firstChild.className = "MJXp-right MJXp-scale" + o;
					r.firstChild.style.marginLeft = pre.Em(p * (o / 10 - 1) / q * 100);
					p = p * o / 10;
					s.firstChild.style.borderTopWidth = pre.Em(0.08 / Math.sqrt(o / 10));
				}
				s.appendChild(n);
				u.appendChild(r);
				u.appendChild(s);
		...
		h.mroot.Augment({
			toPreviewHTML: function (q) {
				...
				var t = this.PHTMLlayoutRoot(html.Element("span"), q.firstChild);
				n.className = "MJXp-script";
				var u = parseInt(t.firstChild.firstChild.style.fontSize);
				var o = 0.55 * (u / 120) + p.d * 0.8, s = -0.6 * (u / 120);
				if (u > 150) { s *= 0.95 * Math.ceil(150 / u * 10) / 10 }
				n.style.marginRight = pre.Em(s);
				n.style.verticalAlign = pre.Em(o);
				if (-s > p.w * 0.8) { n.style.marginLeft = pre.Em(-s - p.w * 0.8) }
				q.appendChild(n);
				q.appendChild(t);
		...
		h.mtable.Augment({
			toPreviewHTML: function (E) {
				...
				for (F = 0, A = N.length; F < A; F++) {
					N[F] = pre.length2em(N[F]);
				}
				for (F = 0, A = w.length; F < A; F++) {
					w[F] = pre.length2em(w[F]);
				}
				var K = html.Element("span");
				while (E.firstChild) { K.appendChild(E.firstChild) }
				E.appendChild(K);
				var y = 0, s = 0;
				for (F = 0, A = this.data.length; F < A; F++) {
					var v = this.data[F];
					if (v) {
						var J = pre.arrayEntry(w, F - 1),
							C = pre.arrayEntry(t, F);
						...
						for (D = 0, z = v.data.length; D < z - B; D++) {
							var p = v.data[D + B];
							if (p) {
								var M = pre.arrayEntry(N, D - 1),
									G = pre.arrayEntry(L, D);
								var I = p.PHTMLspanElement();
								if (D) { x.w += M; I.style.paddingLeft = pre.Em(M) }
								if (F) { I.style.paddingTop = pre.Em(J) }
								I.style.textAlign = G;
							}
						}
						y += x.h + x.d;
...
function FAST_PREVIEW(hub, html, browser) {
	...
	var d = MathJax.Extension["fast-preview"] = {
		...
		postFilter: function (j, i) {
			if (!i.math.root.toPreviewHTML) {
				...
				hub.RestartAfter(h.Push({}));
			}
			i.math.root.toPreviewHTML(j);
		},
		Register: function (h) {
			hub.Register.StartupHook(h + " Jax Require", function () {
