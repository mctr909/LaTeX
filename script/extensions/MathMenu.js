(function (f, o, q, e, r) {
    ...
    var h = MathJax.Object.Subclass({
        Keydown: function (u, v) {
            switch (u.keyCode) {
                case b.ESCAPE: this.Remove(u, v); break;
                case b.RIGHT: this.Right(u, v); break;
                case b.LEFT: this.Left(u, v); break;
                case b.UP: this.Up(u, v); break;
                case b.DOWN: this.Down(u, v); break;
                case b.RETURN:
                case b.SPACE: this.Space(u, v); break;
                default: return;
                    break
            }
            return n(u)
    ...
    var g = MathJax.Menu = h.Subclass({
        ...
        Post: function (M, E, B) {
            ...
            if (g.isMobile) {
                o.addElement(v, "span", {
                    ...
                    ontouchend: n,
                    onmousedown: g.Close, onmouseup: n
                }, [["span", {}, "\u00D7"]]);
            }
            I.appendChild(v);
            this.posted = true;
        ...
        IndexOfN: function (x, v) {
            for (var w = 0, u = this.items.length; w < u; w++) {
                if (this.items[w].name[x] === v) { return w }
            }
            return null;
        },
        Right: function (u, v) {
        ...
        Background: function (v) {
            var w = o.addElement(document.body,
                "div",
        ...
        MoveHorizontal: function (y, z, w) {
            if (!y.shiftKey) { return }
            var v = g.AllNodes();
            var u = v.length;
            if (u === 0) { return }
            var x = v[g.Mod(w(g.IndexOf(v, g.CurrentNode())), u)];
            if (x === g.CurrentNode()) { return }
            g.menu.Remove(y, z);
        ...
        Right: function (u, v) {
            g.MoveHorizontal(u, v, function (w) { return w + 1 });
        },
        Left: function (u, v) {
            g.MoveHorizontal(u, v, function (w) { return w - 1 });
        },
        ...
        Mod: function (u, v) { return ((u % v) + v) % v },
        IndexOf: (
            Array.prototype.indexOf ? function (u, v, w) {
    ...
    var c = g.ITEM = h.Subclass({
        ...
        DeactivateSubmenus: function (z) {
            ...
            for (var w = 0, u = v.length; w < u; w++) {
                var x = v[w].menuItem;
                if (x && x.submenu && x.submenu.posted && x !== z.menuItem) { x.Deactivate(v[w]) }
            }
    ...
    g.ENTRY = g.ITEM.Subclass({
        ...
        Up: function (v, u) {
            this.MoveVertical(v, u, function (w) { return w - 1 });
        },
        Down: function (v, u) {
            this.MoveVertical(v, u, function (w) { return w + 1 });
        },
    ...
    g.ITEM.CHECKBOX = g.ENTRY.Subclass({
        ...
        Init: function (v, u, w) {
            if (!i(v)) { v = [v, v] }
            this.name = v; this.variable = u; this.With(w);
        },
        Label: function (v, w) {
    ...
    g.About = function (y) {
        ...
        var x = o.addElement(
            g.About.div, "div", { id: "MathJax_About", tabIndex: 0, onkeydown: g.About.Keydown },
    ...
    g.About.GetFormat = function () {
        var u = MathJax.Hub.outputJax["jax/mml"][0] || {};
        if (u.id !== "HTML-CSS" || !u.webFonts || u.imgFonts) { return }
    ...
    g.ShowSource.Window = function (v) {
        if (!g.ShowSource.w) {
            var w = [], u = s.windowSettings; for (var x in u) {
        ...
        if (g.isMobile) {
            ...
            setTimeout(function () {
                var B = (u.outerHeight - u.innerHeight) || 30,
                    A = (u.outerWidth - u.innerWidth) || 30, w, E;
                ...
                if (g.prototype.msieHeightBug) { B += 35 }
                u.resizeTo(A, B);
                var D;
                try {
                    D = x.screenX;
    ...
    g.Renderer = function () {
        ...
        if (v[0] !== s.settings.renderer) {
                    ...
                    if (!s.settings.warnedMML) {
                        if (y.isChrome && y.version.substr(0, 3) !== "24.") {
                            ...
                            if (y.isSafari && !y.versionAtLeast("5.0")) {
                                ...
                                if (y.isMSIE) {
                                    if (!y.hasMathPlayer) { x = u.MML.MSIE }
                                } else {
                                    if (y.isEdge) {
                                        x = u.MML.WebKit;
                                    } else {
            ...
            if (x) {
                ...
                g.cookie.renderer = v[0].id; g.saveCookie();
                if (!confirm(x)) {
                    g.cookie.renderer = s.settings.renderer = o.Cookie.Get("menu").renderer; g.saveCookie();
                    return;
                }
                if (w) {
    ...
    g.LoadLocale = function () {
        ...
        if (u) {
            ...
            q.Require(u, function (v) {
                if (v != q.STATUS.OK) {
                    lert(t("BadData", "Failed to load translation data from %1", u));
                }
            });
        }
    };
    ...
    g.CreateLocaleMenu = function () {
        ...
        for (var x = 0, u = v.length; x < u; x++) {
            ...
            z.items.push(
                c.RADIO([v[x], y], "locale", { action: g.Locale })
            );
        }
    ...
    f.Register.StartupHook("End Config", function () {
        ...
        g.getCookie();
        g.menu = g(
            c.SUBMENU(["Show", "Show Math As"],
                c.COMMAND(["Original", "Original Form"],
                    g.ShowSource, { nativeTouch: true }
                ),
            ...
            c.RULE(),
            c.SUBMENU(["Settings", "Math Settings"],
                c.SUBMENU(["ZoomTrigger", "Zoom Trigger"],
                ...
                c.SUBMENU(["ZoomFactor", "Zoom Factor"],
                    c.RADIO("125%", "zscale"),
                    c.RADIO("133%", "zscale"),
                    c.RADIO("150%", "zscale"),
                    c.RADIO("175%", "zscale"),
                ...
                c.RULE(),
                c.SUBMENU(["Renderer", "Math Renderer"],
                    { hidden: !s.showRenderer },
                    c.RADIO(["HTML-CSS", "HTML-CSS"], "renderer", { action: g.Renderer }),
                    c.RADIO(["CommonHTML", "Common HTML"], "renderer", { action: g.Renderer, value: "CommonHTML" }),
                    c.RADIO(["PreviewHTML", "Preview HTML"], "renderer", { action: g.Renderer, value: "PreviewHTML" }),
                    c.RADIO(["MathML", "MathML"], "renderer", { action: g.Renderer, value: "NativeMML" }),
                    c.RADIO(["SVG", "SVG"], "renderer", { action: g.Renderer }),
                    c.RADIO(["PlainSource", "Plain Source"], "renderer", { action: g.Renderer, value: "PlainSource" }),
                    c.RULE(),
                    c.CHECKBOX(["FastPreview", "Fast Preview"], "FastPreview")
                ),
                c.SUBMENU("MathPlayer",
                    {
                        hidden: !f.Browser.isMSIE || !s.showMathPlayer,
                        disabled: !f.Browser.hasMathPlayer
                    },
                    c.LABEL(["MPHandles", "Let MathPlayer Handle:"]),
                    c.CHECKBOX(["MenuEvents", "Menu Events"], "mpContext", { action: g.MPEvents, hidden: !m }),
                    c.CHECKBOX(["MouseEvents", "Mouse Events"], "mpMouse", { action: g.MPEvents, hidden: !m }),
                    c.CHECKBOX(["MenuAndMouse", "Mouse and Menu Events"], "mpMouse", { action: g.MPEvents, hidden: m })
                ),
                c.SUBMENU(["FontPrefs", "Font Preference"], { hidden: !s.showFontMenu },
                ...
                c.SUBMENU(["ContextMenu", "Contextual Menu"], { hidden: !s.showContext },
                ...
                c.CHECKBOX(["Discoverable", "Highlight on Hover"], "discoverable", { hidden: !s.showDiscoverable })
            ),
            c.SUBMENU(["Accessibility", "Accessibility"],
                c.CHECKBOX(["AssistiveMML", "Assistive MathML"], "assistiveMML", { action: g.AssistiveMML }),
                c.CHECKBOX(["InTabOrder", "Include in Tab Order"], "inTabOrder")
            ),
            c.SUBMENU(["Locale", "Language"], { hidden: !s.showLocale, ltr: true },
        ...
        if (g.isMobile) {
            (function () {
                ...
                if (v.zoom === "Hover" || v.zoom == "Click") { v.zoom = "None" }
    ...
    g.showMathPlayer = function (u) {
        g.cookie.showMathPlayer = s.showMathPlayer = u; g.saveCookie();
